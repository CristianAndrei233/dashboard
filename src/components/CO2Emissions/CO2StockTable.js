...
const CO2StockTable = () => {
    ...
    csv(csvUrl)
      .then(csvData => {
        const formattedData = csvData.reduce((acc, d) => {
          ...
          acc[d.Entity][d.Year] = +d["Annual CO₂ emissions (per capita)"];
          return acc;
        }, {});
  ...
  return (
    <TableContainer>
      <Title>Per capita CO₂ emissions by Year</Title>
      <SearchInput
        type="text"
      ...
      {selectedCountry && (
        ...
        >
          <ModalContent>
            ...
            <p>Year: {year}</p>
            <p>CO₂ Emissions: {selectedCountry[year].toLocaleString()} tonnes per capita</p>
